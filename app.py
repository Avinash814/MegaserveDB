...
from margin_shortfall import margin_shortfall_bp
            ...
            for file in files:
                ...
                try:
                    ...
                    if table_name_lower == 'users' and 'dte' in df.columns:
                        ...
                        dte_mapping = {'0': '0DTE', '1': '1DTE', '2': '2DTE', '3': '3DTE', '4': '4DTE'}
...
def view_table(table):
    ...
    try:
        with engine.connect() as connection:
            ...
            if not sort_column or sort_column not in columns:
                sort_column = columns[0]  # Default to first column
            if sort_direction not in ['asc', 'desc']:
                sort_direction = 'asc'

            # Ensure positive page and rows_per_page
            page = max(1, page)
            rows_per_page = max(1, min(rows_per_page, 3000))  # Cap at 3000 to prevent abuse
            ...
            # Render template for non-AJAX request
...
def manage_database(table):
    ...
    try:
        with engine.connect() as connection:
            ...
            if not table:
                return render_template('rename.html', tables=tables, table_name='', columns=[],
                                     data=[],
            ...
            if not columns:
                flash(f"No columns found for table {table}", "error")
                return render_template('rename.html', tables=tables, table_name=table, columns=[],
                                     data=[],
            ...
            if request.method == 'POST':
                try:
                    ...
                    # Delete Rows
                        ...
                        for row_id in rows_to_delete:
                            try:
                                ...
                                if row_id_int > 0:  # Ensure positive numeric ID
                                    valid_rows.append(row_id_int)
                            except (ValueError, TypeError):
                                continue  # Skip invalid values like 'undefined' or non-numeric
                        if not valid_rows:
                    ...
                    # Bulk Update
                    elif request.form.get('bulk_update'):
                        column = request.form.get('column')
                    ...
                    # Bulk Delete
                    elif request.form.get('bulk_delete'):
                        column = request.form.get('column')
                    ...
                    # Bulk Insert
                    elif request.form.get('bulk_insert'):
                    ...
                    # Add Column
                    elif request.form.get('add_column'):
                    ...
                    # Modify Column
                    elif request.form.get('modify_column'):
                        column = request.form.get('column')
                        data_type = request.form.get('data_type')
                        if not column or column not in columns:
                            flash("Invalid column name", "error")
                    ...
                    # Import Data
                    elif request.form.get('import_data') and 'file' in request.files:
                        file = request.files['file']
            ...
            return render_template('rename.html', tables=tables, table_name=table, columns=columns,
                                 data=data,
                                 primary_key=primary_key,
...
app.register_blueprint(margin_shortfall_bp)
