...
def process_aggregate_data(excluded_users, engine, export=False, use_latest_date=False, selected_date=None):
    """
    Reusable function to process aggregate data for a given date and excluded users.
    If use_latest_date is True, selects the latest date from the users table.
    Returns data for rendering or JSON/CSV export, including unique algo/server counts and top/least users.
    """
    try:
        with engine.connect() as conn:
            ...
            if not required_columns.issubset(summary.columns):
                ...
                return None, None, None, None, [], [], 0, [], [], None, f"Required columns missing: {missing_cols}"
            ...
            # Calculate unique algo and server counts
            ...
            # Calculate top and least users (20% based on Return Ratio)
            ...
            top_count = max(1, int(user_count * 0.2))  # At least 1 user
...
@admin_bp.route('/aggregate', methods=['GET', 'POST'])
def admin_aggregate_page():
    logger.info(f"Accessing admin_aggregate_page, Session: {session}")
    ...
    if not engine:
        ...
        return render_template('admin_aggregate.html', role=session.get('role'), data=None, total_mtm=None, num_users=None, servers=None, selected_date=selected_date, excluded_users=excluded_users, all_user_ids=[])
    ...
    try:
        with engine.connect() as conn:
            ...
            if selected_date:
                try:
                    ...
                    return render_template('admin_aggregate.html', role=session.get('role'), data=None, total_mtm=None, num_users=None, servers=None, selected_date=selected_date, excluded_users=excluded_users, all_user_ids=all_user_ids)
        ...
        return render_template('admin_aggregate.html', role=session.get('role'), data=data, total_mtm=total_mtm, num_users=num_users, servers=servers, selected_date=selected_date, excluded_users=excluded_users, all_user_ids=all_user_ids)
...
@admin_bp.route('/aggregate/total_mtm', methods=['POST'])
def admin_total_mtm():
    logger.info(f"Accessing admin_total_mtm, Session: {session}")
    if 'role' not in session or session.get('role', '') not in ['admin', 'user'] or not session['authenticated']:
        logger.info("Redirecting to login due to failed session check")
        flash("Please log in to access this page", "error")
        return redirect(url_for('login.login'))
    ...
    try:
        ...
        return redirect(url_for('admin.admin_aggregate_page', selected_date=selected_date))
...
@admin_bp.route('/aggregate/include_users', methods=['POST'])
def include_users():
...
@admin_bp.route('/aggregate/reset_exclusions', methods=['POST'])
...
@admin_bp.route('/analysis', methods=['GET', 'POST'])
    ...
    try:
        if request.method == 'POST':
            ...
            if request.form.get('export') == 'csv' and 'processed_data_path' in session:
                try:
                    ...
                    if not os.path.exists(data_path):
                        ...
                        return redirect(url_for('admin.admin_analysis_page'))
            ...
            if 'file_upload' not in request.files:
                ...
                return render_template('admin_analysis.html', role=session.get('role'))
        ...
        return render_template('admin_analysis.html', role=session.get('role'), data=data,
...
# @admin_bp.route('/analysis_fallback', endpoint='admin_analysis')
# def admin_analysis_fallback():
#     referrer = request.referrer or "Unknown"
#     logger.error(f"Caught incorrect reference to 'admin.admin_analysis'. Referrer: {referrer}, Request Path: {request.path}, Full Request: {request.url}, Args: {request.args}")
#     flash("Redirected from an outdated or incorrect analysis link. Please update references to 'admin_analysis_page'. Check logs for the referrer URL.", "warning")
#     return redirect(url_for('admin.admin_analysis_page'))
