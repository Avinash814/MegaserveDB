...
def fetch_data():
    ...
    try:
        with engine.connect() as connection:
            ...
            logger.info(f"Fetched {len(df)} rows from users table")
            logger.debug(f"Date column unique values: {df['date'].unique()}")
            logger.debug(f"Server column unique values: {df['server'].unique()}")
            logger.debug(f"DTE column unique values: {df['dte'].unique()}")
            return df
...
def format_indian_number(number):
    ...
    try:
        ...
        if is_negative:
            integer_part = integer_part[1:]  # Remove the negative sign temporarily
        
        # Reverse the integer part for easier processing
        integer_part = integer_part[::-1]
        # First group: last 3 digits (or fewer)
        formatted = integer_part[:3]
        # Remaining groups: every 2 digits
        ...
        if is_negative:
            formatted = '-' + formatted
        # Add decimal part with 2 digits
        return f"{formatted}.{decimal_part:0>2}"
    except (ValueError, AttributeError):
        return "0.00"

def process_data(df, time_period='Last Day', total_mtm_algo=None, total_mtm_servers=None, top_least_algo=None, top_least_servers=None, selected_user_id=None, dte_filter=None):
    """Process data for the dashboard with filtering and aggregation."""
    ...
    try:
        ...
        logger.debug(f"Date column after conversion: {df['date'].dtype}")
        ...
        logger.debug(f"After deduplication - Unique algos: {unique_algos}")
        logger.debug(f"After deduplication - Unique servers: {unique_servers}")
        logger.debug(f"After deduplication - Unique user IDs: {all_user_ids}")
        logger.debug(f"After deduplication - Unique DTEs: {unique_dtes}")
        ...
        if dte_filter and dte_filter != 'Overall':
            ...
            if time_period in ['Today', 'Last Day', 'Yesterday']:
                ...
                if filtered_df.empty and time_period == 'Yesterday':
                    # Fallback to Today for Yesterday
                ...
                # Display only the first and last dates of available data
            ...
            # Handle "Overall" case: no DTE filtering
        ...
        logger.info(f"Total Trading Days (Summary): {total_trading_days_summary}")

        # Compute algo-to-servers mapping
        ...
        logger.debug(f"Algo to Servers mapping: {algo_to_servers}")
        ...
        # Calculate total allocation as sum of average allocations per user
        user_avg_alloc = summary_df.groupby('user_id')['allocation'].mean()
        total_allocation = user_avg_alloc.sum()
        total_mtm = summary_df['mtm_all'].sum()
        total_return_percent = round((total_mtm / total_allocation ), 2) if total_allocation != 0 else 0.0

        # Calculate Total AUM for Summary
        ...
        mtm_vs_day_df = base_df.copy()
        if selected_user_id and selected_user_id != "All Users":
            mtm_vs_day_df = mtm_vs_day_df[mtm_vs_day_df['user_id'] == selected_user_id]
        if total_mtm_algo and total_mtm_algo != "All Algos":
            mtm_vs_day_df = mtm_vs_day_df[mtm_vs_day_df['algo'] == total_mtm_algo]
        if total_mtm_servers and "All Servers" not in total_mtm_servers:
            mtm_vs_day_df = mtm_vs_day_df[mtm_vs_day_df['server'].isin(total_mtm_servers)]
        ...
        if not mtm_vs_day_df.empty:
            ...
            # Calculate Total AUM for filtered data
            user_avg_alloc = mtm_vs_day_df.groupby('user_id')['allocation'].mean()
            total_allocation = user_avg_alloc.sum()
            ...
            # Calculate Win Day and Loss Day
        ...
        # --- Top/Least Performing Users and Detailed Report ---
        ...
        # Sum the average allocations per user for each algo-server group
...
def dashboard_route():
    ...
    logger.info(f"Dashboard request with filters: time_period={time_period}, total_mtm_algo={total_mtm_algo}, total_mtm_servers={total_mtm_servers}, top_least_algo={top_least_algo}, top_least_servers={top_least_servers}, user_id={selected_user_id}, dte_filter={dte_filter}")
    ...
    result = process_data(df, time_period, total_mtm_algo, total_mtm_servers, top_least_algo, top_least_servers, selected_user_id, dte_filter)
    ...
    final_df, grand_total, num_users, num_algos, all_user_ids, unique_server_count, top_users, least_users, unique_algos, unique_servers, time_period, selected_date, dte_message, date_display, top_users_count, least_users_count, total_return_percent, mtm_dates, mtm_values, worst_trading_day, best_trading_day, win_rate, total_mtm_value, profit_factor, algo_to_servers, total_aum, total_trading_days, win_day, loss_day = result
